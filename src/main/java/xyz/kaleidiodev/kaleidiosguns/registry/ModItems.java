...
public class ModItems {
	...
	public static void registerItems(final RegistryEvent.Register<Item> event) {
		...
		slagGatling = initItem(reg, new GatlingItem(defP().durability(KGConfig.slagSmgDurability.get()), 0, KGConfig.slagSmgDamageMultiplier.get(), KGConfig.slagSmgFireDelay.get(), KGConfig.slagSmgInaccuracy.get(), KGConfig.slagSmgEnchantability.get(), KGConfig.slagSmgSwitchSpeed.get(), KGConfig.slagSmgMeleeDamage.get()).repair(() -> Ingredient.of(Tags.Items.INGOTS_NETHERITE)), "slag_smg").projectileSpeed(KGConfig.slagSmgProjectileSpeed.get()).fireSound(ModSounds.slag_smg).setIsSensitive(true).setIsLava(true);
